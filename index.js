...
/*function cpu() {
    ...
    // IA
    let move = findWinningMove(jogo.matriz, 5);
    if (!move) {
        move = findWinningMove(jogo.matriz, 1);
        if (!move) {
...
function fimJogo() {
    ...
    if (fim) {
        ...
        setTimeout(escolherCPU, 2000);  // agenda a jogada da CPU para 2 segundos a partir desse momento
}
...
let cpuAtiva; // Variável para armazenar a CPU ativa

// Função que representa a CPU inteligente
function cpuInteligente() {
  // Lógica da CPU inteligente
  console.log("CPU Inteligente ativada");
  // Inserir a lógica da CPU inteligente aqui
  let lin, col;
...
// Função que representa a CPU aleatória
function cpuAleatoria() {
  // Lógica da CPU aleatória
  console.log("CPU Aleatória ativada");
  // Inserir a lógica da CPU aleatória aqui
  do {   // gera na matriz (lin, col) uma posição vazia (0)
    lin = Math.floor(Math.random() * 3);  // gera uma linha de 0 a 2
    col = Math.floor(Math.random() * 3);  // gera uma coluna de 0 a 2
} while (jogo.matriz[lin][col] !== 0);  // enquanto diferente de vazio (0)
...
// Função para escolher qual CPU ativar no início do jogo
function escolherCPU() {
  const probabilidade = Math.random(); // Gere um número aleatório entre 0 e 1

  if (probabilidade <= 0.7) {
    cpuAtiva = cpuInteligente; // Ativa a CPU inteligente com 70% de chance
  } else {
    cpuAtiva = cpuAleatoria; // Ativa a CPU aleatória com 30% de chance
  }
  cpuAtiva(); // Chama a CPU ativa
}

// Exemplo de uso no início do jogo
//escolherCPU(); // Chama a função para escolher a CPU no início do jogo

// O jogo continua com a CPU ativa até o fim do jogo
// Quando o jogo terminar, você pode chamar escolherCPU() novamente para escolher uma CPU no próximo jogo.